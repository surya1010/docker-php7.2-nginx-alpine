...
RUN sed -i "s/;date.timezone =.*/date.timezone = Asia\/Jakarta/g" /etc/php7.4/php.ini \
    ...
    && sed -i "s/user = nobody/user = root/g" /etc/php7.4/php7.4-fpm.d/www.conf \
    && sed -i "s/group = nobody/group = root/g" /etc/php7.4/php7.4-fpm.d/www.conf \
    && sed -i "s/listen.owner = nobody/listen.owner = root/g" /etc/php7.4/php7.4-fpm.d/www.conf \
    && sed -i "s/listen.group = nobody/listen.group = root/g" /etc/php7.4/php7.4-fpm.d/www.conf \
    && sed -i "s/listen.group = nobody/listen.group = root/g" /etc/php7.4/php7.4-fpm.d/www.conf
